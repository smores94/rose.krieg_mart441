...
function enterPhase(phaseNum, spawnCount, obstacleCount = 0) {
...
class Player extends GameObject {
  ...
  phaseUnlockMessage = `Phase ${phaseNum} Unlocked!`;
  ...
  collectibles = [];
  spawnCollectibles(phaseNum, spawnCount);
  if (obstacleCount > 0) spawnDangerObstacles(obstacleCount);

  // Phase-specific one-off logic:
  switch (phaseNum) {
    case 2:
      // e.g. play a “level up” audio
      if (sounds.level) sounds.level.play();
      break;

    case 3:
      // spawn additional obstacles
      // (we already did obstacleCount)
      if (sounds.level) sounds.level.play();
      break;
    ...
    case 6:
      // Exiting mirror world into Fairyland
    ...
    case 8:
      ...
      break;
                ...
                if (currentPhase === 3
                    && phase3Collected >= PHASE3_COUNT
                    && score >= PHASE3_SCORE_REQUIRED) {
                  enterPhase(3, 20, /* maybe obstacles? */ 0);
                ...
                if (currentPhase === 4
                    && phase4Collected >= PHASE4_COUNT
                    && score >= PHASE4_SCORE_REQUIRED) {
                  enterPhase(5, 25, /* maybe obstacles? */ 0);
                phaseStartTime = Date.now();
                timeWarningPlayed = false;
                ...
                if (currentPhase === 5
                    && phase5Collected >= PHASE5_COUNT
                    && score >= PHASE5_SCORE_REQUIRED) {
                  enterPhase(6, 30, /* maybe obstacles? */ 0);
                }
                ghostPlayer = new GhostPlayer(player.x, player.y);
                currentPhase = 5;
                phaseStartTime = Date.now();
